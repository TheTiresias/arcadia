...
date: 2026-01-02
subtitle: Stories, chapters, navigation, and custom colors
tags: [docs]
---

Fiction lives in `fiction/`. Each story is a directory containing a metadata file and one markdown file per chapter. Create a new story with:

```
arcadia new fiction <slug>
```

This creates `fiction/{slug}/story.md` and a starter `chapter-01.md`.
...
| `tags`             | No       | List of tags; generates tag pages and shows tag links on the ToC page |
...
## Sections and Margin Notes

A horizontal rule `---` in a chapter becomes a `<section>` break, identical to posts. Sidenotes (`^[text]`) and margin notes (`>[text]`) are also available:
